import numpy as np

test_icon = 1
if test_icon == 0:
    ###################################
    #inner_loop test
    # a = np.random.choice(range(pow(2,3074)))
    a = 
    # b = np.random.choice(range(pow(2,54)))
    b = 0b00110000_00110011_00101101_00110010_11010011_10000100_11101000
    # b = hex(b)
    # print(b)
    print(hex(a*b))
elif test_icon == 1:
    ###################################
    #multiplier test  
    # radix = 54 
    a1 = 0xdbe90cfb52f766
    a2 = 0xd3_3b1c_9b7c_2ffc
    bi = 0b00110000_00110011_00101101_00110010_11010011_10000100_11101000
    a11 =a1 & 0x3ffff
    a12 =(a1 & 0xffffc0000) >> 18
    a13 =a1 >> 36
    a21 =a2 & 0x3ffff
    a22 =(a2 & 0xffffc0000) >> 18
    a23 =a2 >> 36
    b1 =bi & 0x3ffff
    b2 =(bi & 0xffffc0000) >> 18
    b3 =bi >> 36
    print("***************")
    print(hex(a11))
    print(hex(a12))
    print(hex(a13))
    print(hex(a21))
    print(hex(a22))
    print(hex(a23))
    print(hex(b1))
    print(hex(b2))
    print(hex(b3))
    print("***************")
    print(hex(a11*b1))
    print(hex(a11*b2))
    print(hex(a11*b3))
    print(hex(a12*b1))
    print(hex(a12*b2))
    print(hex(a12*b3))
    print(hex(a13*b1))
    print(hex(a13*b2))
    print(hex(a13*b3))
    print("***************")
    print(hex(a21*b1))
    print(hex(a21*b2))
    print(hex(a21*b3))
    print(hex(a22*b1))
    print(hex(a22*b2))
    print(hex(a22*b3))
    print(hex(a23*b1))
    print(hex(a23*b2))
    print(hex(a23*b3))
    print("***************")
    print(hex(a1*bi))
    print(hex(a2*bi))
    print("***************")
    print(hex(bi))
    print(hex(a1))
    print(hex(a2))
    print(hex((a2*bi)>>110))
    print(hex((a1*bi) >> 54))
    print(hex(((a1*bi)>>108)<<54))